...
Purpose: Vocabulary-size, sequence-length, and token coverage diagnostics.
...
MODULE_PURPOSE = 'Vocabulary-size, sequence-length, and token coverage diagnostics.'
...
def token_coverage(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def vocab_curve(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = token_coverage(materialized)
...
def sequence_length_stats(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
