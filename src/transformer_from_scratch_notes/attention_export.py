...
Purpose: Attention-map export helpers for review examples.
...
MODULE_PURPOSE = 'Attention-map export helpers for review examples.'
...
def attention_summary(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def salient_tokens(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = attention_summary(materialized)
...
def export_attention_case(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
